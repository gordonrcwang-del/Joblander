...
## Canonical `position_intro` format

*This section is the single source of truth for this format. The `interview-prep` skill writes the same structure to `<Company>/<Position>/position_intro.md` and points here rather than restating it — edit the format here and both skills follow. Only the destination differs: chat reply here, file there.*
