...
會自己開瀏覽器,網址是 `http://127.0.0.1:8765/`,`Ctrl-C` 結束。沒有通行碼,網址固定,可以直接存書籤。
...
1. **登入不會自動跳分頁。** launchd 起的是伺服器,不是瀏覽器。要看就點啟動器,或開你存的書籤。
2. **點了啟動器卻跳出「現在沒有在跑」。** 伺服器掉了而且沒被拉回來。對話框上按「叫起來」就好;連按兩次還是這樣,看 `automation/dashboard/_internal/logs/launchd.log`。

**看板沒有任何驗證。** 綁 `127.0.0.1` 就是全部的防護 —— 同一台機器上的其他程式打得到它。這是 2026-08-29 明確做的取捨,不是漏掉的。
