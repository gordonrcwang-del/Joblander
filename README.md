...
跑起來會自己開瀏覽器,網址是 `http://127.0.0.1:8765/`。沒有通行碼 —— 綁 `127.0.0.1` 就是全部的防護。網址固定,**存書籤就行**。
...
安裝時會順手在 `~/Applications` 做一個啟動器(Finder 裡叫「求職看板」),**把它拖到 Dock,以後點一下就開**。它比書籤多做一件事:先確認伺服器活著,沒活著就問要不要叫起來。單獨重做:
